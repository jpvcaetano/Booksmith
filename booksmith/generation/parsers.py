    ...
    def _parse_characters_fallback(response: str) -> List[Character]:
        ...
        for line in lines:
            ...
            if (
                ...
                if current_character.get("name"):
                    ...
                    try:
                        char = Character(
                            ...
                            background_story=current_character.get(
                                "background", "No background provided"
                            ),
                            appearance=current_character.get(
                                "appearance", "No description provided"
                            ),
                            personality=current_character.get(
                ...
                # Start new character
                current_character = {
                    "name": line.replace("*", "").replace("Character:", "").strip()
                }
...
class StructuredResponseParser:
    ...
    def parse_story_summary(response: Union[str, Dict[str, Any]]) -> str:
        ...
        validation_result = StructuredOutputValidator.validate_and_parse(
            response, "story_summary"
        )
        ...
        if validation_result.success:
            ...
            if isinstance(validation_result.data, dict):
                ...
                if "continuity_notes" in validation_result.data:
                    logger.info(
                        f"Chapter continuity notes: {validation_result.data['continuity_notes']}"
                    )
                if "character_development" in validation_result.data:
                    logger.info(
                        f"Character development notes: {validation_result.data['character_development']}"
                    )
