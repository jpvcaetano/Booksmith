...
def create_epub_file(book: Book, output_path: Path) -> None:
    ...
    try:
        ...
        raise ImportError(
            "ebooklib is required for EPUB generation. Install it with: pip install ebooklib"
        )
    ...
    title_chapter = epub.EpubHtml(
        title="Title Page", file_name="title.xhtml", content=title_content
    )
    ...
    for i, chapter in enumerate(content_chapters):
        ...
        next_chapter = (
            content_chapters[i + 1] if i < len(content_chapters) - 1 else None
        )
